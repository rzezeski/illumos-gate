...
void
mac_dummy_pass_test(ktest_ctx_hdl_t *ctx)
{
...
void
mac_dummy_fail_test(ktest_ctx_hdl_t *ctx)
{
	KT_ASSERT3S(mt_dummy(5), ==, 0, ctx);
	ktest_result_pass(ctx);
}

void
mac_dummy_err_test(ktest_ctx_hdl_t *ctx)
{
	KT_EASSERT0(mt_dummy(3), ctx);
}

void
mac_dummy_skip_test(ktest_ctx_hdl_t *ctx)
{
	ktest_result_skip(ctx, "The king stay the king.");
}

void
mac_dummy_input_test(ktest_ctx_hdl_t *ctx)
{
	uchar_t *bytes;
	size_t num_bytes = 0;
	...
	if (num_bytes < 4) {
		ktest_result_error(ctx, "expected 4 or more bytes, got %u",
		    num_bytes);
	}
...
/*
 * TODO: mac_sw_cksum_ipv4() does full checksum of ULP only. I'll want
 * to write another test for mac_sw_cksum() for full/partial checkum
 * and to verify IP header checksum.
 * TODO: UDP test.
 * TODO: Split across b_cont.
 */
void
mac_sw_cksum_ipv4_tcp_test(ktest_ctx_hdl_t *ctx)
{
	...
	eh = mt_ether_hdr(mp, 0, "f2:35:c2:72:26:57", "92:ce:5a:29:46:9d",
	    ETHERTYPE_IP);
	/* TODO mt_copy_data() which reads headers to copy data to
 ...
 * bytes. It is the responsbility of the caller to free the copy. It
    ...
    snoop_pkt_record_hdr_t **rhdr)
{
	...
	/*
	 * A corrupted record or trunccated stream could point us past
	 * the end of the stream.
	 */
	if (PAST_END(itr, sizeof (snoop_pkt_record_hdr_t))) {
		ktest_result_error(ctx, "record corrupted or stream truncated, "
		    "read past end of stream for record header #%d: "
		    "0x%p + %u > 0x%p", itr->sic_pkt_num, itr->sic_pos,
	...
	if (PAST_END(itr, ntohl(itr->sic_pkt_hdr->spr_record_len))) {
		ktest_result_error(ctx, "record corrupted or stream truncated, "
		    "read past end of stream for record #%d: 0x%p + %u > 0x%p",
    ...
    snoop_iter_t **itr_out)
{
	...
	*itr_out = NULL;
	...
	if (input_len < 16) {
		ktest_result_error(ctx, "snoop stream truncated at file "
		    "header: %u < %u (%s:%d)", input_len, 16, __FILE__,
		    __LINE__);
		return (ENOBUFS);
	}

	if (memcmp(input, &id, sizeof (id)) != 0) {
		ktest_result_error(ctx, "snoop stream malformed "
		    "identification: %x %x %x %x %x %x %x %x", input[0],
	...
	if (version != 2) {
		ktest_result_error(ctx, "snoop stream bad version: %u != %u",
		    version, 2);
		return (EINVAL);
	}
	...
	if (datalink != 4) {
		ktest_result_error(ctx, "snoop stream bad datalink type: "
		    "%u != %u", datalink, 4);
...
mac_sw_cksum_ipv4_snoop_test(ktest_ctx_hdl_t *ctx)
{
	...
	if (at_least_one) {
		...
		ktest_msg_clear(ctx);
		ktest_result_error(ctx,
		    "at least one TCP/IPv4 packet expected");
	}
...
_init()
{
	...
	ret = ktest_create_module("mac", "mac", &km);
	VERIFY0(ret);
	ret = ktest_create_suite("checksum", &ks, NULL, NULL);
	VERIFY0(ret);
	ret = ktest_add_test(ks, "mac_sw_cksum_ipv4_tcp_test",
	    mac_sw_cksum_ipv4_tcp_test, KTF_NONE);
	ret = ktest_add_test(ks, "mac_sw_cksum_ipv4_bad_proto_test",
	    mac_sw_cksum_ipv4_bad_proto_test, KTF_NONE);
	ret = ktest_add_test(ks, "mac_sw_cksum_ipv4_snoop_test",
	    mac_sw_cksum_ipv4_snoop_test, KTF_INPUT);
	VERIFY0(ret);
	ktest_add_suite(km, ks);

	ret = ktest_create_suite("dummy", &ks, NULL, NULL);
	VERIFY0(ret);
	ret = ktest_add_test(ks, "mac_dummy_pass_test", mac_dummy_pass_test,
	    KTF_NONE);
	VERIFY0(ret);
	ret = ktest_add_test(ks, "mac_dummy_fail_test", mac_dummy_fail_test,
	    KTF_NONE);
	VERIFY0(ret);
	ret = ktest_add_test(ks, "mac_dummy_err_test", mac_dummy_err_test,
	    KTF_NONE);
	VERIFY0(ret);
	ret = ktest_add_test(ks, "mac_dummy_skip_test", mac_dummy_skip_test,
		KTF_NONE);
	VERIFY0(ret);
	ret = ktest_add_test(ks, "mac_dummy_input_test", mac_dummy_input_test,
		KTF_INPUT);
	VERIFY0(ret);
	ktest_add_suite(km, ks);
