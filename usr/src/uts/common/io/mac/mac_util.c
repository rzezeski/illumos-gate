    ...
    const mac_ether_offload_info_t *meoi)
{
	...
	if (MBLKL(target_mp) < hdr_len_reqd || DB_REF(target_mp) > 1 ||
	    !OK_32PTR(mp->b_rptr + l3_off)) {
		const size_t pad_by = (4 - (l3_off % 4)) % 4;
    ...
    const uint32_t encap_len, const mac_ether_offload_info_t *inner_info)
{
	 ...
	 * header is actively read. Ensure the headers are contiguous and that
	 * the innermost L3 and L4 headers are 4B aligned. The IP header is
	 * used only for the benefit of DTrace SDTs, whereas the TCP header is
	 * actively read.
	 *
	...
	if (MBLKL(omp) < hdr_len_reqd || !OK_32PTR(omp->b_rptr + oehlen)) {
