...
ena_admin_get_basic_stats(ena_t *ena)
{
	...
	enahw_resp_basic_stats_t *resp_stats = &resp.erd_resp.erd_basic_stats;
	...
	cmd_stats->ecgs_device_id = 0xFFFF;

	mutex_enter(&ena->ena_lock);
	VERIFY0(ena_admin_submit_cmd(ena, &cmd));
	VERIFY0(ena_admin_read_resp(ena, &resp));

	ena->ena_basic_stats.ebs_tx_bytes =
	    ((uint64_t)resp_stats->erbs_tx_bytes_high << 32) |
	    (uint64_t)resp_stats->erbs_tx_bytes_low;
	ena->ena_basic_stats.ebs_tx_pkts =
	    ((uint64_t)resp_stats->erbs_tx_pkts_high << 32) |
	    (uint64_t)resp_stats->erbs_tx_pkts_low;
	ena->ena_basic_stats.ebs_tx_drops =
	    ((uint64_t)resp_stats->erbs_tx_drops_high << 32) |
	    (uint64_t)resp_stats->erbs_tx_drops_low;

	ena->ena_basic_stats.ebs_rx_bytes =
	    ((uint64_t)resp_stats->erbs_rx_bytes_high << 32) |
	    (uint64_t)resp_stats->erbs_rx_bytes_low;
	ena->ena_basic_stats.ebs_rx_pkts =
	    ((uint64_t)resp_stats->erbs_rx_pkts_high << 32) |
	    (uint64_t)resp_stats->erbs_rx_pkts_low;
	ena->ena_basic_stats.ebs_rx_drops =
	    ((uint64_t)resp_stats->erbs_rx_drops_high << 32) |
	    (uint64_t)resp_stats->erbs_rx_drops_low;
...
ena_m_stat(void *arg, uint_t stat, uint64_t *val)
{
	...
	case MAC_STAT_NORCVBUF:
		*val = ena->ena_basic_stats.ebs_rx_drops;
		break;

	case MAC_STAT_RBYTES:
		*val = ena->ena_basic_stats.ebs_rx_bytes;
		break;

	case MAC_STAT_IPACKETS:
		*val = ena->ena_basic_stats.ebs_rx_pkts;
		break;

	case MAC_STAT_OBYTES:
		*val = ena->ena_basic_stats.ebs_tx_bytes;
		break;

	case MAC_STAT_OPACKETS:
		*val = ena->ena_basic_stats.ebs_tx_pkts;
		break;
