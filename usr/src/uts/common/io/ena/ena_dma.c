...
ena_dma_adminq_attr(ena_t *ena, ddi_dma_attr_t *attrp, size_t size)
{
	 ...
	 */
	ASSERT3U(ena->ena_hw->eh_dma_width, >=, 32);
	ASSERT3U(ena->ena_hw->eh_dma_width, <=, 48);
	attrp->dma_attr_addr_lo = 0x0;
	attrp->dma_attr_addr_hi = ENA_DMA_BIT_MASK(ena->ena_hw->eh_dma_width);
	 ...
	 */
	attrp->dma_attr_align = ENA_DMA_ADMINQ_ALIGNMENT;
...
/*
 * Write the physical DMA address, phys_addr, to the destination
 * values. This function assumes that phys_addr does not exceed the
 * device's DMA width, which should be checked during DMA allocation.
 */
void
ena_set_dma_addr(const uint64_t phys_addr, uint32_t *dst_low,
    uint16_t *dst_high)
{
