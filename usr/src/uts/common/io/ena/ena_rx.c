...
ena_refill_rx(ena_rxq_t *rxq, uint16_t num)
{
	...
	while (num != 0) {
		 ...
		 */
		desc->length = rcb->ercb_dma.edb_len;
		desc->req_id = tail_mod;
		VERIFY3P(rcb->ercb_dma.edb_cookie, !=, NULL);
		ena_set_dma_addr(rcb->ercb_dma.edb_cookie->dmac_laddress,
		    &desc->buff_addr_lo, &desc->buff_addr_hi);
		...
		ena_xxx(rxq->er_ena, "length: 0x%x reserved2: 0x%x ctrl: 0x%x "
		    "req_id: 0x%x reserved6: 0x%x buff_addr_lo: 0x%x "
		    "buff_addr_hi: 0x%x reserved16_w3: 0x%x", desc->length,
		    desc->reserved2, desc->ctrl, desc->req_id, desc->reserved6,
		    desc->buff_addr_lo, desc->buff_addr_hi,
		    desc->reserved16_w3);
		rxq->er_sq_tail_idx++;
...
ena_ring_rx_start(mac_ring_driver_t rh, uint64_t gen_num)
{
	...
	/* TODO temorary to test DMA */
	ddi_dma_attr_t attr;
	ddi_device_acc_attr_t acc;
	ena_dma_io_attr(ena, &attr, 4096);
	ena_dma_acc_attr(ena, &acc);
	ena_dma_buf_t edb;
	if (!ena_dma_alloc(ena, &edb, &attr, &acc, 4096, B_FALSE))
		ena_xxx(ena, "failed to DMA alloc 4K (CONSISTENT)");

	ena_dma_free(&edb);

	ena_dma_io_attr(ena, &attr, 4096 * 2);
	if (!ena_dma_alloc(ena, &edb, &attr, &acc, 4096 * 2, B_FALSE))
		ena_xxx(ena, "failed to DMA alloc 8K (CONSISTENT)");

	ena_dma_free(&edb);

	ena_dma_io_attr(ena, &attr, 4096 * 3);
	if (!ena_dma_alloc(ena, &edb, &attr, &acc, 4096 * 3, B_FALSE))
		ena_xxx(ena, "failed to DMA alloc 12K (CONSISTENT)");

	ena_dma_free(&edb);

	ena_dma_io_attr(ena, &attr, 4096 * 4);
	if (!ena_dma_alloc(ena, &edb, &attr, &acc, 4096 * 4, B_FALSE))
		ena_xxx(ena, "failed to DMA alloc 16K (CONSISTENT)");

	ena_dma_free(&edb);

	if (!ena_dma_alloc(ena, &edb, &attr, &acc, 4096, B_TRUE))
		ena_xxx(ena, "failed to DMA alloc 4K (STREAMING)");

	ena_dma_free(&edb);

	ena_dma_io_attr(ena, &attr, 4096 * 2);
	if (!ena_dma_alloc(ena, &edb, &attr, &acc, 4096 * 2, B_TRUE))
		ena_xxx(ena, "failed to DMA alloc 8K (STREAMING)");

	ena_dma_free(&edb);

	ena_dma_io_attr(ena, &attr, 4096 * 3);
	if (!ena_dma_alloc(ena, &edb, &attr, &acc, 4096 * 3, B_TRUE))
		ena_xxx(ena, "failed to DMA alloc 12K (STREAMING)");

	ena_dma_free(&edb);

	ena_dma_io_attr(ena, &attr, 4096 * 4);
	if (!ena_dma_alloc(ena, &edb, &attr, &acc, 4096 * 4, B_TRUE))
		ena_xxx(ena, "failed to DMA alloc 16K (STREAMING)");
...
ena_ring_rx(ena_rxq_t *rxq, int poll_bytes)
{
	...
	while (ENAHW_RX_CDESC_PHASE(cdesc) == rxq->er_cq_phase) {
		...
		rcb->ercb_offset = cdesc->offset;
		rcb->ercb_length = cdesc->length;
		if (cdesc->offset != 0) {
			ena_xxx(ena, "Rx cdesc first: %u last: %u offset: %u "
			    "length: %u", first, last, rcb->ercb_offset,
			    rcb->ercb_length);
		}
		...
		bcopy(rcb->ercb_dma.edb_va, mp->b_wptr, rcb->ercb_length);
		...
		if (ena->ena_hw->eh_rx_l3_ipv4_csum &&
		...
		if(ena->ena_hw->eh_rx_l4_ipv4_full_csum &&
		    l4csum_checked &&
