    ...
    mac_ether_offload_info_t *meo, size_t mlen)
{
	...
	ASSERT3U(hdr_len, <=, txq->et_ena->ena_hw->eh_tx_max_hdr_len);
...
ena_ring_tx(void *arg, mblk_t *mp)
{
	...
	ena_xxx(ena, "Tx desc len_ctrl: 0x%x", desc->len_ctrl);
	ena_xxx(ena, "Tx desc meta_ctrl: 0x%x", desc->meta_ctrl);
	...
	ena_xxx(ena, "Tx desc buf_addr_lo: 0x%x", desc->buff_addr_lo);
...
ena_tx_intr_work(ena_txq_t *txq)
{
	...
	while ((cdesc->flags & 0x1) == txq->et_cq_phase) {
		mblk_t *mp;
