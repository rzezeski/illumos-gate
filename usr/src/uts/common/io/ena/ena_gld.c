...
/*
 * The ENA driver delays I/O allocation and setup until ring start,
 * leaving mc_start(9E) with little to do.
 */
static int
...
/*
 * The ENA driver delays I/O teardown until ring stop, leaving
 * mc_stop(9E) with little to do.
 */
static void
ena_m_stop(void *arg)
{
...
{
	/* Promisc? Where we're going we don't need promisc. */
	return (0);
}
    ...
    uint_t pr_valsize, const void *pr_val)
{
	...
	ena_xxx(ena, "setprop %s %u", pr_name, pr_num);
    ...
    uint_t pr_valsize, void *pr_val)
{
	...
	mutex_enter(&ena->ena_lock);
	ena_xxx(ena, "getprop %s %u", pr_name, pr_num);
...
{
	ena_t *ena = arg;
	ena_xxx(ena, "propinfo %s %u", pr_name, pr_num);
}
