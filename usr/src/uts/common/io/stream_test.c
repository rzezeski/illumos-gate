...
_init()
{
	...
	ret = ktest_create_module("stream", "stream", &km);
	VERIFY0(ret);
	ret = ktest_create_suite("mblk", &ks, NULL, NULL);
	VERIFY0(ret);
	ret = ktest_add_test(ks, "mblkl_test", mblkl_test, KTF_NONE);
	VERIFY0(ret);
	ret = ktest_add_test(ks, "msgsize_test", msgsize_test, KTF_NONE);
	VERIFY0(ret);
