...
regdump(struct adapter *sc, void *data, int flags)
{
	...
	if (t4_cver_eq(sc, CHELSIO_T4)) {
		...
		/* Regdump size is same for both T5 and T6 */
		if (r.len > T5_REGDUMP_SIZE)
			r.len = T5_REGDUMP_SIZE;
		else if (r.len < T5_REGDUMP_SIZE)
			return (ENOBUFS);
	}
	...
	if (t4_cver_eq(sc, CHELSIO_T4)) {
		...
		reg_ranges = &t6_reg_ranges[0];
		arr_size = ARRAY_SIZE(t6_reg_ranges);
		buf_size = T6_REGDUMP_SIZE;
	}
